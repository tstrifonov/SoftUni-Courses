function towns(input) {
    ...
    for (let info of input) {
        let infoHolder = info.split(" | ");
        let town = infoHolder[0];
        let latitude = Number(infoHolder[1]).toFixed(2);
        let longitude = Number(infoHolder[2]).toFixed(2);

        let newTown = {town, latitude, longitude};
        townsArray.push(newTown);
    }
