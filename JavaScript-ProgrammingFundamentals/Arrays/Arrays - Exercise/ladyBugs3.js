function ladyBugs(input) {
    ...
    let bugsPosition = input[1].split(" ");
    let testedBugs = [];
   //в testedBugs вкарвам проверените индекси на буболечки (между 0 и по-малки от дължината на полето)
   for (let i = 0; i < bugsPosition.length; i++) {
        if (Number(bugsPosition[i]) >= 0 && Number(bugsPosition[i]) < fieldSize) {
            testedBugs.push(Number(bugsPosition[i]));
        }
   }

    //създавам поле с буболечки
    let field = [];
    let flag = undefined;
    for (let i = 0; i < fieldSize; i++) {
       flag = false;
        for (let u = 0; u < testedBugs.length; u++) {
            if (i == testedBugs[u]) {
                flag = true;
            }
        }
    ...
    //започвам да следвам командите
    for (let i = 2; i < input.length; i++) {
        let arr = input[i].split(" ");
        let ladybugIndex = Number(arr[0]);
        let direction = arr[1];
        let flyLength = Number(arr[2]);
        
        if (field[ladybugIndex] !== 1) {
            continue;
        } else {
            field[ladybugIndex] = 0; //EMPTY
           ...
           if (direction == "right" && flyLength >= 0) {
            ...
            } while (flag == false && ladybugIndex + flyLength < field.length);
           } 
           
           else if (direction == "right" && flyLength < 0) {
            let flag = false
            do {
                if (field[ladybugIndex + flyLength] == 0) {
                    field[ladybugIndex + flyLength] = 1;
                ...
                ladybugIndex += flyLength;
            } while (flag == false && ladybugIndex + flyLength >= 0);
           } 
...
ladyBugs([ 3, '0 5 1 -6', '0 right 1', '2 right 1' ]); //0 1 0
